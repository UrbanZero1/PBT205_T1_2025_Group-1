...
namespace MyApp
{
    internal static class tracker
    {
        static async Task Main(string[] args)
        {
            ...
            // Test data
            /*
            board.AddPerson("Alice", 1, 1, true);
            ...
            */
            ...
            consumer.ReceivedAsync += (modal, ea) =>
            {
                ...
                if (routingKey.StartsWith("new.person"))
                {
                    ...
                    if (parts.Length == 5 && int.TryParse(parts[2], out int xPos) && int.TryParse(parts[3], out int yPos) && bool.TryParse(parts[4], out bool infected))
                    {
                        board.AddPerson(name, xPos, yPos, infected);
                    }
                    else
                    {
            ...
            Thread newWindowThread = new Thread(() =>
            {
                ...
                BoardViewWindow newWindow = new BoardViewWindow(ref board);
                app.Run(newWindow);
            });
    ...
    public class Board
    {
        ...
        public void AddPerson(string name, int xPos, int yPos, bool infected = false)
        {
            ...
            Person newPerson = new Person() { name = name, contacts = new List<Person>(), infected = infected };
            ...
            for (int i = 0; i < people.Count; i++)
            {
                ...
                for (int j = 0; j < people.Count; j++)
                {
                        ...
                        person.currentCell.y == other.currentCell.y)
                    {
                        ...
                        // Check if the other person is already in the contacts list
